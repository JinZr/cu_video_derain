...
class GeneratorState(nn.Module):
    def __init__(self, latent_size=64, state_size=128, motion_size=64, num_feature=128):
        ...
        self.linear1 = nn.Sequential(
            nn.Linear(latent_size + state_size, num_feature, bias=True), nn.ReLU(True)
        )
        if motion_size > 0:
            self.linear2 = nn.Sequential(
                nn.Linear(motion_size, num_feature, bias=True), nn.ReLU(True)
            )
        self.linear3 = nn.Sequential(
            nn.Linear(num_feature, state_size, bias=True), nn.Tanh()
        )
