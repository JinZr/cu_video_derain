...
from .refinenet_dict import refinenet_dict
...
class CoarseNet(nn.Module):
    def __init__(
        ...
        if self.use_bidirect:
            ...
            self.bidirection_fusion = nn.Conv2d(
                6, 3, kernel_size=5, stride=1, padding=2, bias=True
            )
    ...
    def forward(self, x):
        ...
        if self.use_bidirect:
            bwd_out = self.R_bwd(
                torch.cat((rev_maps(x_decoder, b, d), rev_maps(x_mff, b, d)), 1), b, d
            )

            concat_cube = cubes_2_maps(torch.cat((fwd_out, bwd_out.flip(dims=[2])), 1))[
                0
            ]
...
class FineNet(nn.Module):
    def __init__(
        ...
        HR_conv0 = B.conv_block(
            nf, nf, kernel_size=3, norm_type=norm_type, act_type=act_type
        )
        HR_conv1 = B.conv_block(
            nf, out_nc, kernel_size=3, norm_type=norm_type, act_type=None
        )

        self.net = B.sequential(
            c3d,
            B.ShortcutBlock_ZKH(B.sequential(*rb_blocks), LR_conv),
            HR_conv0,
            HR_conv1,
        )
    ...
    def forward(self, x):
        ...
        inp_6c_F = torch.cat((x, x), 1)
        return self.net(inp_6c_F) + x[:, :, x.shape[2] // 2, :, :]
